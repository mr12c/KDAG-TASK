  ...
  return (
    ...
    <div className='flex flex-col gap-2 w-[30%]'>
        {data?.content.map((item,index)=> 
         <div  onClick={()=>{setCurrentWeak(index)}} className={`cursor-pointer  smb p-2 w-[60%]  ${currentWeak===index ? "hmb" : ""} justify-center rounded flex items-center gap-2 bg-red-50`}><i class="fa-solid fa-calendar-days"></i><div>Week {index+1}</div></div>
      )}
    </div> 
    <div className='w-[50%] gap-2 flex flex-col *:rounded-lg'>
        {data?.content[currentWeak].map((item)=> <div className='p-2 smb hover:scale-[1.01]'>
